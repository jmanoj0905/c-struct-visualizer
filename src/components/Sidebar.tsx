...
export const templates = {
  singlyLinkedList: {
    ...
    description: "Linear chain with next pointers",
    icon: List,
    structs: [
      {
        name: "Node",
  ...
  doublyLinkedList: {
    ...
    description: "Bidirectional with prev/next",
    icon: List,
    structs: [
      {
        name: "DNode",
  ...
  tree: {
    name: "Binary Tree",
    description: "Hierarchical left/right structure",
    icon: GitBranch,
    structs: [
      {
        name: "TreeNode",
  ...
  graph: {
    name: "Graph",
    description: "Network with multiple edges",
    icon: Network,
    structs: [
      {
        name: "GraphNode",
  ...
  circularList: {
    name: "Circular List",
    description: "Loop back to start node",
    icon: Network,
    structs: [
      {
        name: "CircNode",
  ...
  bst: {
    name: "BST",
    description: "Binary search tree with parent",
    icon: GitBranch,
    structs: [
      {
        name: "BSTNode",
  ...
  setTimeout(() => {
    ...
    if (templateKey === "singlyLinkedList") {
      ...
      setTimeout(() => {
        ...
        if (inst1 && inst2) {
          addConnection({ sourceInstanceId: inst1.id, sourceFieldName: "next", targetInstanceId: inst2.id });
          addConnection({ sourceInstanceId: inst2.id, sourceFieldName: "prev", targetInstanceId: inst1.id });
        }
        if (inst2 && inst3) {
          addConnection({ sourceInstanceId: inst2.id, sourceFieldName: "next", targetInstanceId: inst3.id });
          addConnection({ sourceInstanceId: inst3.id, sourceFieldName: "prev", targetInstanceId: inst2.id });
        }
      }, 100);
      ...
      setTimeout(() => {
        ...
        if (inst1 && inst2) {
          addConnection({ sourceInstanceId: inst1.id, sourceFieldName: "edges[0]", targetInstanceId: inst2.id });
        }
        if (inst1 && inst3) {
          addConnection({ sourceInstanceId: inst1.id, sourceFieldName: "edges[1]", targetInstanceId: inst3.id });
        }
        if (inst2 && inst4) {
          addConnection({ sourceInstanceId: inst2.id, sourceFieldName: "edges[0]", targetInstanceId: inst4.id });
        }
      }, 100);
      ...
      setTimeout(() => {
        ...
        if (inst3 && inst4) {
          addConnection({ sourceInstanceId: inst3.id, sourceFieldName: "next", targetInstanceId: inst4.id });
        }
        if (inst4 && inst1) {
          addConnection({ sourceInstanceId: inst4.id, sourceFieldName: "next", targetInstanceId: inst1.id });
        }
      }, 100);
      ...
      setTimeout(() => {
        ...
        if (rootInst && leftInst) {
          addConnection({ sourceInstanceId: rootInst.id, sourceFieldName: "left", targetInstanceId: leftInst.id });
          addConnection({ sourceInstanceId: leftInst.id, sourceFieldName: "parent", targetInstanceId: rootInst.id });
        }
        if (rootInst && rightInst) {
          addConnection({ sourceInstanceId: rootInst.id, sourceFieldName: "right", targetInstanceId: rightInst.id });
    ...
    <div className="h-full w-64 bg-white border-r-4 border-black flex flex-col">
      ...
      >
        ...
        <button
          ...
          className="p-1.5 border-2 border-black rounded-base hover:translate-x-1 hover:translate-y-1 transition-transform shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-none"
