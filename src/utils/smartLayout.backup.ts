...
/**
 * Detect circular linked list (all nodes form a single cycle)
 */
function detectCircularLinkedList(
  nodeIds: Set<string>,
  connections: PointerConnection[],
  _instances: StructInstance[],
  _structDefinitions: CStruct[],
  ...
  // For circular list: each node has exactly 1 outgoing and 1 incoming edge
  const outgoingCount = new Map<string, number>();
  const incomingCount = new Map<string, number>();

  componentConnections.forEach((c) => {
    outgoingCount.set(c.sourceInstanceId, (outgoingCount.get(c.sourceInstanceId) || 0) + 1);
    incomingCount.set(c.targetInstanceId, (incomingCount.get(c.targetInstanceId) || 0) + 1);
  });

  // All nodes must have exactly 1 outgoing and 1 incoming edge
  for (const nodeId of nodeIds) {
    if (outgoingCount.get(nodeId) !== 1) return { matches: false, confidence: 0 };
    if (incomingCount.get(nodeId) !== 1) return { matches: false, confidence: 0 };
  }

  // Must have exactly N edges for N nodes (forms a complete cycle)
  if (componentConnections.length !== nodeIds.size) return { matches: false, confidence: 0 };

  return { matches: true, confidence: 0.95 };
}

/**
 * Detect graph with cycles (has cycles but not a circular list)
 */
function detectGraphWithCycles(
  nodeIds: Set<string>,
  connections: PointerConnection[],
  _instances: StructInstance[],
  _structDefinitions: CStruct[],
  ...
  // Must have cycles (SCCs with size > 1)
  const hasCycles = graphMetrics.sccs.some((scc) => scc.ids.size > 1);
  if (!hasCycles) return { matches: false, confidence: 0 };

  // Must NOT be a circular linked list (that's detected separately)
  const circularList = detectCircularLinkedList(nodeIds, connections, _instances, _structDefinitions);
  if (circularList.matches) return { matches: false, confidence: 0 };

  return { matches: true, confidence: 0.85, metadata: { sccs: graphMetrics.sccs } };
}
...
function layoutLayeredDAG(
  ...
  function assignDepth(nodeId: string, depth: number) {
    // If already visited with a shallower depth, skip
    if (nodeLayer.has(nodeId)) {
      const existingDepth = nodeLayer.get(nodeId)!;
...
function layoutGraphWithCycles(
  ...
  const nonCycleNodes = Array.from(component.nodeIds).filter((id) => !cycleNodes.has(id));

  // Layout cycle nodes in a circle
  ...
  if (cycleCount > 0) {
    const baseRadius = config.circularRadius;
    const radiusPerNode = 150;
    const radius = Math.max(baseRadius, cycleCount * radiusPerNode);
    const angleStep = (2 * Math.PI) / cycleCount;

    cycleNodeArray.forEach((id, index) => {
      const angle = -Math.PI / 2 + index * angleStep;
      positions.set(id, {
        x: radius * Math.cos(angle),
        y: radius * Math.sin(angle),
  ...
  if (nonCycleNodes.length > 0) {
    // Find nodes that connect to the cycle (roots of non-cycle part)
    const rootsOfNonCycle = nonCycleNodes.filter((nodeId) => {
      const outgoingToCycle = componentConnections.some(
        (c) => c.sourceInstanceId === nodeId && cycleNodes.has(c.targetInstanceId),
      );
      return outgoingToCycle;
    });

    // Use topological-style layering for non-cycle nodes
    const visited = new Set<string>();
    const layers: string[][] = [];
    const nodeLayer = new Map<string, number>();
    ...
    function assignLayer(nodeId: string, layer: number) {
      ...
      const children = componentConnections
        .filter((c) => c.sourceInstanceId === nodeId && nonCycleNodes.includes(c.targetInstanceId))
    ...
    componentConnections.forEach((c) => {
      if (nonCycleNodes.includes(c.sourceInstanceId) && nonCycleNodes.includes(c.targetInstanceId)) {
        nodesWithNonCycleIncoming.add(c.targetInstanceId);
      }
    });

    const trueRoots = nonCycleNodes.filter((id) => !nodesWithNonCycleIncoming.has(id));
    ...
    // Position non-cycle nodes to the left of the circle
    const circleLeftEdge = cycleCount > 0 ? -config.circularRadius - 200 : 0;

    layers.forEach((layer, layerIndex) => {
      const x = circleLeftEdge - layerIndex * (config.nodeWidth + config.horizontalSpacing);
      const totalHeight = layer.length * config.nodeHeight + (layer.length - 1) * config.verticalSpacing;
      let y = -totalHeight / 2;
...
function getPatternDetectors(): PatternRegistry[] {
  return [
    ...
    },
    {
      ...
      detector: detectCircularLinkedList,
      priority: 92,
      description: "Circular linked list where all nodes form a single cycle",
    },
    {
    ...
    {
      pattern: StructurePattern.GRAPH_WITH_CYCLES,
      detector: detectGraphWithCycles,
      priority: 68,
      description: "Graph with cycles (not a circular list)",
    },
    {
...
function getLayoutStrategies(): LayoutStrategyRegistry[] {
  return [
    ...
    {
      pattern: StructurePattern.GRAPH_WITH_CYCLES,
      strategy: layoutGraphWithCycles,
      description: "Hybrid layout: cycles in circle, other nodes hierarchical",
    },
    {
      pattern: StructurePattern.DIRECTED_ACYCLIC_GRAPH,
...
function classifyComponents(
  ...
  // Find connected components using bidirectional adjacency
  ...
  for (const nodeId of allConnectedNodes) {
    ...
    // Detect pattern using registry (this now properly distinguishes circular lists from graphs with cycles)
